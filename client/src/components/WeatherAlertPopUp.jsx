    ...
    ">
      ...
      <div className={`flex items-center justify-between px-4 py-2 rounded-t-xl
        ...
        <div className="flex items-center space-x-2">
          {/* Minimize/Maximize button first */}
          <button
            onClick={() => setMinimized(!minimized)}
            className="text-current text-xl leading-none font-bold pb-2"
            aria-label={minimized ? 'Maximize popup' : 'Minimize popup'}
          >
            {minimized
              ? <span className="font-bold relative top-px">+</span>
              : <span className="font-bold relative top-px">-</span>
            }
          </button>
          {/* Close button next to Minimize */}
          <button
            onClick={onClose}
            className="text-current text-2xl leading-none pb-2"
      ...
      {/* Conditionally render body and footer when not minimized */}
      {!minimized && (
      <>
        ...
        <div className="px-6 pb-4">
          <button
            ...
            className={`
              w-full py-2 rounded-lg text-white pb-2
