// routeGeneratorRoutes.unit.test.js with filter feature

const request = require('supertest');
const express = require('express');
const routeRouter = require('../../routes/routeGeneratorRoutes');

// Mock Google API and loop generation
...
jest.mock('../../controllers/looproute.js', () => jest.fn(() => {
  return Promise.resolve({
    geojson: {
      type: 'LineString',
      coordinates: [
...
describe('Unit Tests - Waypoint Filtering', () => {
  it('should generate route with ordered waypoints and loop extension', async () => {
    ...
    expect(res.body.actualDist).toBeGreaterThan(0);
    expect(res.body.routeCoords.length).toBeGreaterThan(1);
  });

  it('should reject invalid waypoint structure', async () => {
    const res = await request(app)
      .post('/api/route')
      .send({
  ...
  it('should reject waypoint outside Singapore boundary', async () => {
    const res = await request(app)
      .post('/api/route')
      .send({
        start: { lat: 1.3, lng: 103.8 },
